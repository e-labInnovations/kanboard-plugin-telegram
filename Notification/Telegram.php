...
class Telegram extends Base implements NotificationInterface {
    ...
    protected function sendMessage($apikey, $bot_username, $forward_attachments, $telegram_proxy, $chat_id, array $project, $eventName, array $eventData, $is_topic = false, $message_thread_id = 0) {
        ...
        $message .= $this->helper->TelegramHelper->processContent(htmlspecialchars($title, ENT_NOQUOTES | ENT_IGNORE), $project) . "\n";
        ...
        if (in_array($eventName, $subtask_events)) {
            ...
            $attachment = sys_get_temp_dir() . "/kanboard_telegram_plugin/" . $this->helper->TelegramHelper->clean($file_name);
